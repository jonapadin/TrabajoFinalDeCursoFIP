...
=======
    ...
    const actualizarTotalCarrito = () => {
        ...
        btnFinalizar.addEventListener("click", () => {
          ...
          // Validación para Nombre (no vacío, longitud mínima, no solo números y al menos una letra)
          if (nombre.length < 3) {
          ...
          // Validación para Dirección (no vacío, no solo números, y al menos una letra)
          ...
          if (!/[a-zA-Z]/.test(direccion)) {
            alert("La dirección debe contener al menos una letra.");
            return;
          }

          // Validación para Teléfono (9 dígitos)
          const telefonoRegex = /^[0-9]{8}$/; // 9 dígitos, ajusta según lo necesario
          if (!telefonoRegex.test(telefono)) {
            alert("El número de teléfono debe tener 8 dígitos.");
            return;
          }

          // Validación para DNI (debe tener 8 dígitos)
  ...
  const renderCarrito = () => {
    ...
    carrito.forEach((producto) => {
      ...
      clone.querySelector(
        ".descripcion"
      ).textContent = `Producto: ${producto.descripcion}`;
